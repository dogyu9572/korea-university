...
### 0. 퍼블리싱 예시 페이지
프로젝트에 퍼블리싱 작업을 위한 예시 페이지가 준비되어 있습니다.

**접속 방법:**
- 메인: `http://localhost:8000/`
- 서브페이지: `http://localhost:8000/sub/sample`

**파일 구조:**
```
app/Http/Controllers/
└── SubController.php          # 서브페이지 컨트롤러

resources/views/
├── home/
│   └── index.blade.php        # 메인 페이지
└── sub/
    └── sample.blade.php       # 서브페이지 예시

routes/
└── web.php                    # 라우트 정의 (prefix 그룹 방식)
```

**라우트 구조 (routes/web.php):**
```php
// 서브페이지 관련 라우트
Route::prefix('sub')->name('sub.')->group(function () {
    Route::get('/sample', [SubController::class, 'sample'])->name('sample');
});
// 결과: /sub/sample → SubController@sample → resources/views/sub/sample.blade.php
```

**새 서브페이지 추가 방법:**
1. `SubController.php`에 메서드 추가
2. `routes/web.php`의 prefix 그룹 안에 라우트 추가
3. `resources/views/sub/` 폴더에 블레이드 파일 생성
...
| `http://localhost/sub/sample` | `resources/views/sub/sample.blade.php` (퍼블리싱 예시) |
