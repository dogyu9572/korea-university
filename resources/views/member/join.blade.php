...
<main class="sub_wrap member_wrap">
	...
	<div class="inner" @if($errors->any()) data-join-errors="1" @endif>
		<form action="{{ route('member.join.store') }}" method="POST" id="joinForm">
			@csrf
			...
			<div class="dl_slice in_inputs member_area">
				...
				<div class="inputs">
					<dl>
						...
						<dd class="inbtn">
							<input type="email" name="email" placeholder="이메일을 입력해주세요." value="{{ old('email') }}" required>
							<button type="button" class="btn" id="btnCheckEmail">중복확인</button>
						</dd>
						<dd id="emailCheckMsg" class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;display:none;"></dd>
						@error('email')<dd class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;">{{ $message }}</dd>@enderror
						@error('email_verified')<dd class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;">{{ $message }}</dd>@enderror
					</dl>
					<dl>
						<dt>비밀번호<span class="c_blue_light">*</span></dt>
						<dd><input type="password" name="password" class="w100p" placeholder="영문/숫자/특수문자 조합 두가지 이상(8~10자 이내 입력)" required></dd>
					...
					<dl>
						...
						<dd><input type="password" name="password_confirmation" class="w100p" placeholder="비밀번호를 한 번 더 입력해주세요." required></dd>
					...
					<dl>
						...
						<dd class="inbtn">
							<input type="text" name="phone_number" placeholder="휴대폰번호를 입력해주세요." value="{{ \App\Models\Member::formatPhoneForDisplay(old('phone_number')) }}" required>
							<button type="button" class="btn" id="btnCheckPhone">중복확인</button>
						</dd>
						<dd id="phoneCheckMsg" class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;display:none;"></dd>
						@error('phone_number')<dd class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;">{{ $message }}</dd>@enderror
						@error('phone_verified')<dd class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;">{{ $message }}</dd>@enderror
					</dl>
					<dl>
						<dt>이름<span class="c_blue_light">*</span></dt>
						<dd><input type="text" name="name" class="w100p" placeholder="이름을 입력해주세요. (최대 8글자)" value="{{ old('name') }}" maxlength="8" required></dd>
			...
			<div class="dl_slice in_inputs member_area">
				...
				@error('terms_privacy')<p class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;">{{ $message }}</p>@enderror
				@error('terms_service')<p class="join_field_error" style="color:#c00;font-size:0.875rem;margin-top:0.25rem;">{{ $message }}</p>@enderror
			</div>
...
<script src="{{ asset('js/member-join.js') }}"></script>
@endsection
