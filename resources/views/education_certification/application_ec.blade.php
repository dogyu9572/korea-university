...
<main class="sub_wrap inner" id="application-ec-list">
	...
	<div class="search_wrap">
		<form method="GET" action="{{ route('education_certification.application_ec') }}" id="searchForm">
						...
						<dl>
							...
							<dd class="dates">
								...
								<select name="period_year">
									...
									@foreach($periodYears ?? [now()->year, now()->year + 1, now()->year - 1] as $y)
	...
	<ul class="round_tabs mb80">
		<li class="{{ $currentTab === 'all' ? 'on' : '' }}"><a href="{{ route('education_certification.application_ec', array_merge(request()->except('tab', 'page'), ['tab' => 'all'])) }}">전체({{ $tabCounts['all'] }}개)</a></li>
		<li class="{{ $currentTab === 'education' ? 'on' : '' }}"><a href="{{ route('education_certification.application_ec', array_merge(request()->except('tab', 'page'), ['tab' => 'education'])) }}">교육 신청({{ $tabCounts['education'] }}개)</a></li>
		<li class="{{ $currentTab === 'certification' ? 'on' : '' }}"><a href="{{ route('education_certification.application_ec', array_merge(request()->except('tab', 'page'), ['tab' => 'certification'])) }}">자격증 접수({{ $tabCounts['certification'] }}개)</a></li>
		<li class="{{ $currentTab === 'online' ? 'on' : '' }}"><a href="{{ route('education_certification.application_ec', array_merge(request()->except('tab', 'page'), ['tab' => 'online'])) }}">온라인 교육({{ $tabCounts['online'] }}개)</a></li>
	</ul>
	...
	<div class="board_top mt80">
		...
		<div class="right list_filter">
			<a href="{{ route('education_certification.application_ec', array_merge(request()->query(), ['sort' => 'created_at'])) }}" class="{{ $currentSort === 'created_at' ? 'on' : '' }}">최신 등록순</a>
			<a href="{{ route('education_certification.application_ec', array_merge(request()->query(), ['sort' => 'application_start'])) }}" class="{{ $currentSort === 'application_start' ? 'on' : '' }}">신청 기간순</a>
		</div>
	</div>

	<div class="thum_list">
		@forelse($programs as $item)
			@if($item->program_type === 'education')
				@include('education_certification.partials._card_education', ['item' => $item])
			@elseif($item->program_type === 'certification')
				@include('education_certification.partials._card_certification', ['item' => $item])
			@else
				@include('education_certification.partials._card_online', ['item' => $item])
			@endif
		@empty
	...
	@include('education_certification.partials.pagination')
</main>
...
$('.btn_reset').on('click', function () {
	location.href = '{{ route("education_certification.application_ec", ["tab" => $currentTab]) }}';
});
